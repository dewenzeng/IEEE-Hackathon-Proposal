...
# change these folders if you want to us the code on your own machine
model_infection_dir = '/afs/crc.nd.edu/user/d/dzeng2/code/covid19/pretrained_model/model_infection/'
model_lung_dir = '/afs/crc.nd.edu/user/d/dzeng2/code/covid19/pretrained_model/model_lung/'
test_data_dir = '/afs/crc.nd.edu/user/d/dzeng2/code/covid19/dataset/test/'
threshold = 2
batch_size = 2
...
def generate_quantize_model(model):
    ...
    for (layer, qlayer) in zip(model.named_parameters(), qmodel.named_parameters()):
        for tensor, target_tensor in zip(layer[1], qlayer[1]):
            target_tensor.copy_(tensor.detach().clone())
